...
class UploadService extends BaseService
{
     ...
     */
    public function uploadSinglePic(object $file, string $savePath = '',string $is_avatar) : array
    {
        if($is_avatar==1){
            if ($file->getSize() > 1024 * 1024) $this->throwExp(400, 'Image size cannot be larger than 1MB');
        }
        if ($file->getSize() > 3 * 1024 * 1024) $this->throwExp(400, 'Image size cannot be larger than 3MB');
        //得到上传文件的后缀
